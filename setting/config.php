	...
	
{
	...
	public function student_info_select($st_id)
	{
		$student_info_sel = "select * from st_info where st_username='$st_id'";
	...
	public function student_info_display_admin()
	{
		$stud_info_admin = "select * from st_info";
		$stud_info_admin_run = $this->connectdb->query($stud_info_admin);
		return $stud_info_admin_run;
	}

	public function parent_info_display_admin()
	{
		$parent_info_admin = "select * from parent_info";
	...
	public function admin_password_change($admin_password_update,$admin_username)
	{
		$admin_password_update = "update meadmin set admin_password='$admin_password_update' where admin_username='$admin_username'";
		$admin_password_update_run = $this->connectdb->query($hod_password_update);
		return $admin_password_update_run;
	}
		...
		public function lab_info()
	{
		...
		$lab_info_admin = "select * from lab_info";
		$lab_info_admin_run = $this->connectdb->query($lab_info_admin);
		return $lab_info_admin_run;
	}
	
	public function techplan_info()
	{
		
		$sub_info_admin = "select * from techplan";
		$sub_info_admin_run = $this->connectdb->query($sub_info_admin);
		return $sub_info_admin_run;
	}
	public function pra_info()
	{
		
		$pra_info_admin = "select * from pract_info";
		$pra_info_admin_run = $this->connectdb->query($pra_info_admin);
		return $pra_info_admin_run;
	}
	...
	public function edit_studid($stud_id)
	{
		$edit_studid = "select * from st_info where st_id='$stud_id'";
		$edit_studid_run = $this->connectdb->query($edit_studid);
		return $edit_studid_run;
	}
	///////////////// update student info from admin/////////////
	public function update_stud_info($up_fullname,$up_grade,$up_roll,$up_dob,$up_address,$up_district,$up_district,$up_father,$up_mother,$up_contact,$stud_id)
	{
	
		$update_stud_info_select = "update st_info set st_fullname='$up_fullname',st_grade='$up_grade',roll_no='$up_roll',st_dob='$up_dob',st_address='$up_address',st_district='$up_district',st_gender='$up_gender',st_father='$up_father',st_mother='$up_mother',st_parents_contact='$up_contact' where st_id='$stud_id'";
		$update_stud_info_run = $this->connectdb->query($update_stud_info_select);
		return $update_stud_info_run;
	}
	...
	////////// add new parents form admin ////////////////////////
	
	public function add_parent($add_p_name,$add_p_address,$add_p_email,$add_p_contact)
	{
	$add_parent = "insert into parent_info(p_name,p_address,p_email,p_contact) value('$add_p_name','$add_p_address','$add_p_email','$add_p_contact')";
	$add_parent_run = $this->connectdb->query($add_parent);
		return $add_parent_run;
	}
	
	public function add_lab($lab_name,$t_fullname)
	{
	$add_lab = "insert into lab_info(lab_name,t_fullname) value('$lab_name','$t_fullname')";
	$add_lab_run = $this->connectdb->query($add_lab);
		return $add_lab_run;
	}
	public function add_plan($sub_name,$c,$tc,$tm,$date)
	{
	$add_plan = "insert into techplan(sub_name,c,tc,tm,date) value('$sub_name','$c','$tc','$tm','$date')";
	$add_plan_run = $this->connectdb->query($add_plan);
		return $add_plan_run;
	}
	public function add_pra($sub_name,$c,$e,$date)
	{
	$add_pra = "insert into pract_info(sub_name,c,e,date) value('$sub_name','$c','$e','$date')";
	$add_pra_run = $this->connectdb->query($add_pra);
		return $add_pra_run;
	}
	...
	public function st_info_display_admin($class_students_data)
	{
		$st_info_display_admin_select = "select * from st_info where st_grade='$class_students_data'";
		$st_info_display_admin_run = $this->connectdb->query($st_info_display_admin_select);
		return $st_info_display_admin_run;
	}
